...
void Model::setModelsCoefficients() {
    ...
    m_fr_rl_com_velocity << 1.90411373,  0.32359911, -1.03284164,
                            -2.71136757, -1.85828522, 1.63224193,
                            2.26059951, -0.46099432, -6.29614738,
                            -5.60249167, -0.43311937;
    m_fl_rr_com_velocity.resize(11);
    m_fl_rr_com_velocity << 1.79447065,  0.39420662,  0.39651159,
                            -1.72202347, -5.05455658, 4.78436649,
                            -2.46483928,  3.12283597, -0.68061628,
                            -0.92890196, 0.64018526;
}
...
double Model::velocityPrediction(double p_previousVelocityX,
    ...
    if (p_currentFeetConfiguration.fr_rl_swinging) {
        l_predictedVelocity = m_fr_rl_com_velocity * l_modelInput;
    }
    else {
        l_predictedVelocity = m_fl_rr_com_velocity * l_modelInput;
    }

    return l_predictedVelocity;
}
